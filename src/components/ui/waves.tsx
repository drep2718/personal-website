...
interface WavesProps {
  ...
  pointerSize?: number;
  /** Opacity of the whole wave layer (0–1) */
  opacity?: number;
}

export function Waves({
  className = "",
  strokeColor = "rgba(196,30,58,0.55)",
  ...
  const updateMouse = (x: number, y: number) => {
    ...
    if (containerRef.current) {
      containerRef.current.style.setProperty("--wx", `${m.sx}px`);
      containerRef.current.style.setProperty("--wy", `${m.sy}px`);
    }
  };
      ...
      {pointerSize > 0 && (
        <div
          style={{
            ...
            borderRadius: "50%",
            transform:
              "translate3d(calc(var(--wx, -1rem) - 50%), calc(var(--wy, 50%) - 50%), 0)",
